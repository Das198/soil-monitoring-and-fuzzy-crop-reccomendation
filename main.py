...
class SoilMonitoringSystem:
    ...
    def handle_command(self, cmd_str: str) -> None:
        ...
        if cmd == "SET_SLAVES":
            if isinstance(slaves, list):
                self.active_slaves = [int(s) for s in slaves if str(s).isdigit() or isinstance(s, int)]
                logger.info(f"[MODE] Active Slaves dirubah menjadi: {self.active_slaves}")
            return
            
        elif cmd == "START":
            if not self.is_monitoring:
                self.is_monitoring = True
                logger.info(f"[MODE] Beralih ke mode MONITORING. Siklus pembacaan dimulai untuk filter: {self.active_slaves}")
            else:
            ...
            logger.info(f"[ACTION] Tombol GENERATE Ditekan! Mengevaluasi fungsi untuk slaves: {self.active_slaves}")
            if not self.db.is_connected:
                logger.warning("[Fuzzy] Database tidak terhubung, gagal menarik data terbaru.")
                return
            
            if not self.active_slaves:
                logger.warning("[Fuzzy] Tidak ada slave aktif yang dipilih untuk dievaluasi.")
                if self.node_red.is_connected:
                    self.node_red.send_data({"tipe": "rekomendasi_urutan", "data": []}, slave_id=0, timestamp=datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
                return
            
            recent_data = self.db.get_recent_data(slave_ids=self.active_slaves, limit=1)
            if not recent_data:
                logger.warning("[Fuzzy] Tidak ada data di database untuk dievaluasi.")
                return
                
            sensor_data = recent_data[0]
            results = self.fuzzy.get_all_crop_recommendations(sensor_data)
            ...
            if self.node_red.is_connected:
                timestamp_str = sensor_data.get('timestamp', datetime.now())
                ...
                self.node_red.send_data(
                    payload, 
                    slave_id=sensor_data.get('slave_id', 1), 
                    timestamp=timestamp_str
                )
                logger.info("[Fuzzy] Data rekomendasi urutan terkirim ke Node-RED!")
            else:
                logger.warning("[Fuzzy] Node-RED tidak terhubung, tidak dapat mengirim ranking tanaman.")
        else:
    ...
    def start(self) -> bool:
        ...
        if not self.db.connect():
            logger.warning("[WARN] Gagal terhubung ke Database (Evaluasi rekomendasi mungkin gagal)")
